...
.loop:
    lodsb ;загружает байт из памяти по адресу ds:si в регистр al и автоматически увеличивает si
    test al, al
    jz .done
...
kernel_read: ;DAPS
    ...
    db 0
    dw 20 ;врядли вывод строки kernel loaded будет весить больше 1кб так что с запасом
    dw 0x1000 ;через [org 0x1000] укажем в коде ядра
    dw 0x0000
    dq 1 ;отсчет с 0, а первые 512 байт занимает загрузчик

times 510 - ($ - $$) db 0 ;заполняет остаток файла до 512 байт нулями, 510 - (это место - начало файла). 510 - сигнатура 2 бита
